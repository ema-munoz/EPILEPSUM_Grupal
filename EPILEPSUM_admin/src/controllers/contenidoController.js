...
contenido.mostrar = async (req, res) => {
    const idMax = await baseDatosSQL.query("SELECT MAX(idContenido) FROM contenidos")
...
contenido.lista = async (req, res) => {
    const contenidoId = req.user.idUsuario;
    const enlistar = await baseDatosSQL.query("SELECT * FROM listaContenido WHERE usuarioIdUsuario = ?",
        [contenidoId])
...
contenido.editar = async (req, res) => {
    ...
    await baseDatosORM.contenido.findOne({
            where: {
    ...
    await baseDatosORM.detalleContenido.findOne({
        where: {
