...
experiencias.lista = async (req, res) => {
    ...
    const enlistar = await baseDatosSQL.query("SELECT * FROM listaexperiencia WHERE usuarioIdUsuario = ?", [experienciasId])
...
experiencias.editar = async (req, res) => {
    ...
    await baseDatosORM.experiencias.findOne({
            where: {
    ...
    await baseDatosORM.detallesExperiencias.findOne({
            where: {
